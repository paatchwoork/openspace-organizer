...
if __name__ == "__main__":
    ...
    if not filename.endswith(".xlsx"):
        filename += ".xlsx"  # if the file doesn't have an extension, add it

    while not os.path.isfile(
        filename
    ):  # if the file doesn't exist, continue asking
        print(
        ...
        if not filename.endswith(".xlsx"):
            filename += (
                ".xlsx"  # if the file doesn't have an extension, add it
            )

    people = (
        pd.read_excel(filename).iloc[:, 0].tolist()
    )  # read it in a DataFrame and convert back to list
...
If not, it will ask them the values"""
    while True:
        ...
        a = input(
            f"The classroom is initialized with 6 tables that have 4 chairs each, for a total of {nb_t*nb_s} spots.\nDo you want to change that ?\ny/n\n"
        )
