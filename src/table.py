class Seat :
    ...
    METHODS
    set_occupant allows to put a new person at the table and sets its free value to False.
    remove_occcupant allows to remove an occupant from a table and sets its free vale to True.
    ...
    def __str__ (self):
        ...
        return(f"This is a Seat() object, free = {free} and occupant = {occupant}")
...
class Table :
    """Defines a table with its seats (which are Seat objects as defined above) its capacity (i.e. the number of seats).
    ...
    METHODS
    has_free_spot() return True if there is a free spot, and False if there is none
    capacity_left() will return the number of free seats at the table
    assign_seat(name) takes the name of the person, and puts it on the first empty seat of the table
    """
    ...
    def __str__ (self):
        return(f"This is a Table() object, capacity = {capacity} and occupant = {occupant}")

    #Is there a free spot at this table ?
    def has_free_spot (self):
        return any([s.free for s in self.seats])    #returns True is the table has a free seat, otherwise returns False
