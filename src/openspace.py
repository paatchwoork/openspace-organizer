...
class OpenSpace:
    ...
    METHODS
    display prints the status of the table in the terminal, in a nice and neat way
    organize takes a list of people and randomly places them at a table
    store puts the list of people and their placement in a seated.tmp file that will be read by the main.py module
    """
    def __init__ (self, number_of_tables :int, number_of_seats :int, tables :list = []):

        self.number_of_tables=number_of_tables  #initializing number_of_tables
    ...
    def organize (self, names :list):
        ...
        for person in names: #take the each person
            ...
            if nb_seated == (self.number_of_tables * self.number_of_seats):
                self.not_seated = names[nb_seated+1:]   #puts al trhe unseated people in a list that will be read by the store() method

            while True :    #infinite loop that will break if the person finds a table
                assigned_table = randint(0,len(self.tables)-1) #choose a random table
                assigned_seat = randint(0,self.tables[assigned_table].capacity) #choose a random seat on that table

                if self.tables[assigned_table].has_free_spot(): #if the table has a free spot
                    self.tables[assigned_table].assign_seat(person) #put that person there
